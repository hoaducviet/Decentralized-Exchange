...
const options = ['Colleciton name', 'Volume', 'Volume USD', 'Floor', 'Highest', 'Listed', 'Items', 'Owners']
        ...
        <div className='select-none flex flex-col w-full'>
            <Card className='w-full rounded-2xl shadow-md space-y-3 p-[2.5vw] '>
                ...
                <div className='flex flex-row justify-between items-center text-md font-semibold opacity-70 px-3 h-[3vw]'>
                    ...
                    <p className='flex flex-row justify-end w-[10%]'>{options[6]}</p>
                    <p className='flex flex-row justify-end w-[10%]'>{options[7]}</p>
                </div>
                        ...
                        return (
                            <Link key={index} href={`/nfts/${item.name.toLowerCase().replace(/\s+/g, '')}`} className='flex flex-col w-full'>
                                <div className='cursor-pointer hover:bg-secondary/80 rounded-none flex flex-row items-center px-3 h-[4vw]'>
                                    <div className='flex flex-row items-center space-x-[0.6vw] w-[30%] text-md'>
                                        <p>{index + 1}</p>
                                    ...
                                    <div className='flex flex-row justify-end w-[10%]'>{formatNumber(parseFloat(item.volume))}</div>
                                    <div className='flex flex-row justify-end w-[10%]'>${formatNumber(parseFloat(item.volume) * parseFloat(eth?.price || "0"))}</div>
                                    <div className='flex flex-row justify-end w-[10%]'>{item.floor_price}</div>
                                    <div className='flex flex-row justify-end w-[10%]'>{item.highest_price}</div>
                                    <div className='flex flex-row justify-end w-[10%]'>{formatNumber(parseFloat(item.total_listed))}</div>
