        ...
        <Card className="flex flex-col w-full rounded-2xl border-[1px] shadow-md">
            ...
            <div className="flex flex-col w-full max-h-[55vw] overflow-x-auto">
                {transactions.map((item, index) => {
                    ...
                    return (
                        <div key={index} className="cursor-pointer flex flex-row justify-between items-center text-md hover:bg-secondary/80 h-[3.5vw] px-3">
                            <div className="w-[15%] flex flex-row justify-start font-medium">{calculateElapsedTime(transaction.createdAt)}</div>
                            <div className="w-[15%] flex flex-row justify-start">
                                <div>{transaction.type}</div>
                            </div>
                            ...
                            <div className="w-[15%] flex flex-row justify-end">{formatNumber(parseFloat(transaction.amount_lpt ?? ""))}</div>
                            <div className="w-[15%] flex flex-row justify-end">${formatNumber(parseFloat(transaction.price ?? ""))}</div>
