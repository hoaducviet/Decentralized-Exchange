...
export const payFeeCollection = async ({ provider, signer, address, collection }: Props) => {
    const market = await loadMarketNFTContract({ provider: signer, address: addressMarketNFT })
    const balance = await provider.getBalance(address);
    const amount = parseEther(collection.total_fee)
    if (balance < amount) {
        throw new Error("Insufficient balance ether");
    }
    try {
        const nonce = await provider.getTransactionCount(address, 'latest');
        const receipt = await market.payFeeCollection(collection._id, amount, {
            nonce: nonce,
            value: amount
        })
        await receipt.wait()
